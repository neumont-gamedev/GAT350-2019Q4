...
layout (binding = 1) uniform sampler2DShadow shadow_sample;
...
float calculateShadow()
{
	...
	float shadow = textureProj(shadow_sample, fshadowcoord);
...
void main()
{
	...
	//color = texture(texture_sample, ftexcoord) * vec4(ambient + ((diffuse + specular) * shadow), 1.0);
