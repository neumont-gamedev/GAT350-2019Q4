#version 430
	
in vec3 fposition;
in vec3 fnormal;
in vec2 ftexcoord;

out vec4 color;

struct material_s
{
	vec3 ambient;
	vec3 diffuse;
	vec3 specular;
	float shininess; 
};

uniform material_s material;

struct light_s
{
	vec4 position;
	vec3 ambient;
	vec3 diffuse;
	vec3 specular;
};

uniform light_s light;

uniform float dissolve;
uniform vec2 resolution;
uniform vec3 discard_color;

layout (binding = 0) uniform sampler2D texture_sample;
layout (binding = 1) uniform sampler2D fx_texture_sample;

void main()
{
	if (vec3(texture(texture_sample, ftexcoord)) == discard_color) discard;
	//if (floor(gl_FragCoord.y) > 400.0f) discard;

	// scanline
	//if (mod(gl_FragCoord.y, 4.0) > 2.0) discard;

	// dissolve
	float level = texture(fx_texture_sample, ftexcoord).r - dissolve;
	if (level < 0.0) discard;
	vec4 dissolve_color = vec4(1.0f, 1.0, 0.0f, 1.0f) * smoothstep(0.1, 0.0, level);

	vec3 position_to_light = normalize(vec3(light.position) - fposition);

	// ambient
	vec3 ambient = material.ambient * light.ambient;

	// diffuse
	float lDotN = max(0.0, dot(fnormal, position_to_light));
	vec3 diffuse = (material.diffuse * light.diffuse) * lDotN;

	// specular
	vec3 specular = vec3(0.0);
	if (lDotN > 0.0)
	{
		vec3 position_to_view = normalize(-fposition.xyz);
		vec3 reflect_light = reflect(-position_to_light, fnormal);
		float specular_intensity = max(dot(reflect_light, position_to_view), 0.0);
		specular_intensity = pow(specular_intensity, material.shininess);
		specular = light.specular * material.specular * specular_intensity;
	}

	vec4 phong_color = (vec4(ambient + diffuse, 1.0f) * texture(texture_sample, ftexcoord)) + vec4(specular, 1.0f) + dissolve_color;
	//color = color.gggg;
	// color manipulation
	float average = (phong_color.r + phong_color.g + phong_color.b) / 3.0;
	vec4 blend_color = vec4(vec3(average), 1.0);
	color = mix(phong_color, blend_color, gl_FragCoord.y / resolution.y);

	// color = normal
	//color = vec4((fnormal * 0.5) + 0.5, 1.0);
}
