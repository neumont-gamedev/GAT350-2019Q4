...
void phong(in light_s in_light,in vec3 position, in vec3 normal, out vec3 ambient, out vec3 diffuse, out vec3 specular)
{
	vec3 position_to_light;
	if (light.type == POINT || light.type == SPOT) position_to_light = normalize(vec3(in_light.position) - position);
	else if (light.type == DIRECTION) position_to_light = normalize(vec3(-in_light.direction));
	...
	if (light.type == SPOT)
	{
		float cos_angle = dot(-position_to_light, normalize(in_light.direction));
		float angle = acos(cos_angle);
		if (angle < 0.0 || angle >= in_light.cutoff) return;

		spot_intensity = pow(cos_angle, light.exponent);
	}
	...
	if (lDotN > 0.0)
	{
		...
		specular = light.specular * material.specular * specular_intensity * spot_intensity;
	}
}
